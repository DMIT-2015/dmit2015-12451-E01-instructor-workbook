...
public class JakartaPersistenceDataGenerator {
    ...
    public static void generateData(EntityManager em) {
        var faker = new Faker();
        // Seed the database with 32 students
        for (int i = 0; i < 32; i++) {
            var newStudent = Student.of(faker);
            em.persist(newStudent);
        }
        // Seed the database with 7 days of weather forecast
        for (int i = 0; i < 7; i++) {
            var newForecast = WeatherForecast.of(faker);
            em.persist(newForecast);
        }
        // Seed the database with 20 task
        for (int i = 0; i < 20; i++) {
            var newTask = Task.of(faker);
            em.persist(newTask);
        }
